...
const Dropzone = () => {
  ...
  const onDropRejected = () => {
    showAlert(
      "No se pudo subir el archivo, el limite es 500MB, crea una cuenta para subir archivos más grandes."
    );
  };
  ...
  const files = acceptedFiles.map((file) => (
    <li
      className="bg-white flex-1 p-3 mt-4 mb-4 shadow-lg rounded"
      key={file.lastModified}
    >
      <p className="text-lg">
        {file.path.split(".")[0].length > 40
          ? file.path.split(".")[0].slice(0, 30) + "..."
          : file.path.split(".")[0]}
        <span className="text-blue-400">.{file.path.split(".")[1]}</span>
      </p>
    ...
    <div className="md:flex-1 mb-3 mx-2 mt-16 lg:mt-0  flex flex-col items-center justify-center border-dashed border-gray-400 border-2 bg-gray-100 px-4">
      {acceptedFiles.length > 0 ? (
        <div className="mt-10 w-full">
          <h4 className="text-2xl font-bold text-center mb-4 animate__animated animate__bounceIn">
            Archivos
          </h4>
          <ul className="animate__animated animate__bounceIn">{files}</ul>
        ...
        <div {...getRootProps({ className: "dropzone w-full py-32" })}>
          ...
          {isDragActive ? (
            <div className="">
              <p className="text-2xl text-center text-gray-600 py-12">
                {" "}
                Suelta el archivo
              </p>
            </div>
